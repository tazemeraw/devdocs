...
The 3 different parts to write in order to implement a full repository:
...
export const langExample = `$string['pluginname']= 'Example repository';
...
This file contains the main repository class definition. This class must extend the `repository` class. Once this class is created, the plugin will be able to override the parent class methods to alter the way Moodle interact with the repository. See [Administration API](#administration-apis) for more information.

<details>
  <summary>View example</summary>
  <div>

import RepoLib from '!!raw-loader!./_examples/lib.php';

<CodeBlock language="js" title="repository/pluginname/lib.php">{RepoLib}</CodeBlock>
...
export const accessExample = `$capabilities = [
...
## Install a new repository plugin

1. Copy the plugin code of your plugin in a **/repository/** subfolder. For exmaple, **/repository/pluginname**
2. Login as admin on your website and run upgrade
3. Open Site Administration->Plugins->Repositories->Manage Repositories and make your repository **Enabled and visible**
...
- FILE_CONTROLLED_LINK - the file remains in the external repository. By "uploading" it, ownership of the file (in the remote system) is changed so that the [system account in the external repository](https://docs.moodle.org//en/OAuth\_2\_services) becomes the new owner of the file. Later, if the file is accessed, the system account is responsible for granting access to users.

<Tabs>
  <TabItem value="options" label="Setting options example" default>

> Choices offered resulting from the values FILE_INTERNAL, FILE_REFERENCE, and FILE_CONTROLLED LINK, respectively. Whether FILE_EXTERNAL is present is never reflected in this list.
>
...
Optional method that returns the kind of files a repository can store. Returns '\*' for all file types (default implementation), or an array of types or groups (e.g. array('text/plain', 'image/gif', 'web_image')).
...
```
  ...
  <TabItem value="mimetypes" label="Filter mimetypes" default>
...
```
  ...
  <TabItem value="groups" label="Filter file groups" default>
...
1. Define **$string\['enablecourseinstances'\]** and **$string\['enableuserinstances'\]** in your plugin's language file.
2. Add them as items in **get_type_option_names()** function's return array.  Note, you must not define the form fields for these options in the **type_config_form()** function.
...
public static function instance_form_validation($mform, $data, $errors) {
    if (empty($data['email_address'])) {
        $errors['email_address'] = get_string('invalidemailsettingname', 'repository_flickrpublic');
    }
}
```
...
```php title="repository/flickrpublic/lib.php"
<?php
/**
 * repository_flickrpublic class
...
<details>
  <summary>View the full example code</summary>
  <div>

import FlickPublicLib from '!!raw-loader!./_examples/flickrpublic_lib.php';

<CodeBlock language="js" title="repository/flickrpublic/lib.php">{FlickPublicLib}</CodeBlock>
...
First of all, the File Picker using intensively Ajax you will need a easy way to debug.

- Our first question when you write your plugin specification is 'Does the user need to log-in'? If they do, in your plugin you have to detect user session in constructor() function, and use print_login() if required, see more details below.
...
- You want to retrieve the file that the user selected, rewrite get_file() if required, see more details below.
...
import FullList from '!!raw-loader!./_examples/full_list.php';

<CodeBlock language="php">{FullList}</CodeBlock>
...
Returns an array of the elements required in the login form. If no login form is required, then the default implementation of this will redirect to the files list. If $this->options[is not set, then an HTML-snippet with the login fields (but not the form tags) should be output, instead of returning the form details.
...
If your login form is static and never changes, you can add __$ret\['allowcaching'\]('login']) = true;__ and filepicker will not send the request to the server every time user opens the login/search form.
...
```php title="Taken from repository_alfresco"
...
```php title="Example taken from repository_equella"
